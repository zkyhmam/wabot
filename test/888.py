...
async def chat_with_ai(prompt: str, username: str = "مستخدم") -> str:
    ...
    **الأوامر:**
    ...
    try:
        ...
        logger.warning(f"⚠️ Error with GuruSensei API: {e}")
    ...
    try:
        ...
        logger.warning(f"⚠️ Error with Gemini: {e}")
...
class MovieBot:
    def __init__(self):
        ...
        self.operations_state = {
            "share": {"running": False, "from_channel": None, "to_channel": None, "transferred_count": 0, "start_time": None, "task": None, "last_message_id": 0},
            "check": {"running": False, "channel": None, "checked_count": 0, "deleted_count": 0, "start_time": None, "task": None},
    ...
    async def notify_admin(self, message: str, is_search_notification: bool = False):
        try:
            ...
            logger.warning(f"⚠️ FloodWaitError: Waiting {e.seconds} seconds")
            await asyncio.sleep(e.seconds)
            await self.notify_admin(message, is_search_notification)
        except Exception as e:
            logger.error(f"⚠️ Failed to notify admin: {str(e)}")
    ...
    async def scan_channel(self, channel_link: str):
        ...
        try:
            ...
            async for message in self.client.iter_messages(channel_entity, reverse=True):
                ...
                if message.media and isinstance(message.media, MessageMediaDocument) and message.media.document.mime_type.startswith('video/'):
                    ...
                    if video_id not in self.video_db:
                        title = self.clean_title(message.text) if message.text else f"فيديو بدون عنوان (ID: {video_id})"
            ...
            await self.notify_admin(f"**🚫 خطأ أثناء فحص القناة: {str(e)}**")
    ...
    async def check_database(self):
        ...
        self.operations_state["check"]["channel"] = "database"
        ...
        try:
            ...
            for video_id, data in list(self.video_db.items()):
                if not self.operations_state["check"]["running"]:
                    logger.info("⏹ Database check stopped by admin")
                    break
                ...
                if cleaned_title != original_title:
                    ...
                    cleaned_count += 1
                    last_cleaned_title = cleaned_title
                    self.save_video_db()
                ...
                if checked_count % 100 == 0:
                    ...
                    await self.notify_admin(
                        ...
                        f"**🗑️ تم تنظيف: {cleaned_count} عنوان**\n"
                        f"**📌 آخر عنوان منظف: {last_cleaned_title or 'لا يوجد'}**\n"
                        f"**⏳ الوقت المنقضي: {elapsed_time} ثانية**"
                    )
            ...
            await self.notify_admin(f"**🚫 خطأ أثناء تنظيف قاعدة البيانات: {str(e)}**")
            logger.error(f"🚫 Database check error: {str(e)}")
            self.operations_state["check"]["running"] = False

        self.operations_state["check"]["checked_count"] = checked_count
        self.operations_state["check"]["deleted_count"] = cleaned_count

    async def share_videos(self, from_channel: str, to_channel: str):
        ...
        try:
            ...
            logger.info(f"🔄 Starting share from '{from_entity.title}' to '{to_entity.title}' from ID {last_message_id}...")

            async for message in self.client.iter_messages(from_entity, min_id=last_message_id):
            ...
            if self.operations_state["share"]["running"]:
                elapsed_time = int(time.time() - self.operations_state["share"]["start_time"])
                await self.notify_admin(
                    f"**✅ اكتمل النقل من: {from_entity.title} إلى: {to_entity.title}**\n"
                    f"**📊 الإحصائيات النهائية:**\n"
                    f"**✅ تم النقل: {transferred_count} فيديو**\n"
                    f"**⏳ الوقت الكلي: {elapsed_time} ثانية**"
                )
            ...
            await self.notify_admin(f"**🚫 خطأ أثناء النقل: {str(e)}**")
    ...
    async def monitor_new_videos(self, event):
        ...
        if message.media and isinstance(message.media, MessageMediaDocument) and message.media.document.mime_type.startswith('video/'):
            ...
            if self.operations_state["share"]["running"] and self.operations_state["share"]["from_channel"] == self.search_channel:
    ...
    async def process_video_result(self, message: Message, movie_name: str, message_id: int, username: str) -> Tuple[str, str, Message]:
        ...
        if not archive_channel_id:
            logger.warning(f"⚠️ Archive channel not set for @{username}!")
    ...
    async def get_stats(self) -> str:
        ...
        # حساب أحجام الملفات بالكيلوبايت
        ...
        return (
            ...
            f"**🗑️ تم تنظيف: {self.operations_state['check']['deleted_count']} عنوان**\n\n"
            f"**📂 إحصائيات الملفات:**\n"
    ...
    async def initialize(self):
        ...
        if self.operations_state["share"]["running"] and self.share_progress["from_channel"] and self.share_progress["to_channel"]:
            self.operations_state["share"]["task"] = asyncio.create_task(
                self.share_videos(self.share_progress["from_channel"], self.share_progress["to_channel"])
            )
        ...
        if self.operations_state["check"]["running"] and self.operations_state["check"]["channel"] == "database":
            self.scan_status_message = None
            self.operations_state["check"]["task"] = asyncio.create_task(self.check_database())
        ...
        async def handle_commands(event):
            ...
            if re.match(r'^s\s+-ch\s+(\S+)$', text, re.IGNORECASE):
                ...
                if self.operations_state["check"]["running"]:
                    ...
                    await event.respond("**تم إيقاف فحص قاعدة البيانات! ⏹**", parse_mode='markdown', link_preview=False)
                ...
                if self.operations_state["check"]["running"]:
                    await event.respond("**عملية فحص قيد التنفيذ بالفعل! ⏳**", parse_mode='markdown', link_preview=False)
                elif self.operations_state["check"]["channel"] == "database":
                    self.scan_status_message = None
                    self.operations_state["check"]["task"] = asyncio.create_task(self.check_database())
                    await event.respond("**تم استكمال فحص قاعدة البيانات! ▶️**", parse_mode='markdown', link_preview=False)
                    logger.info(f"✅ Check continued by @{username}")
                else:
                    await event.respond("**لا توجد عملية فحص سابقة لاستكمالها! ⚠️**", parse_mode='markdown', link_preview=False)

            elif re.match(r'^share\s+-from\s+(\S+)\s+-to\s+(\S+)$', text, re.IGNORECASE):
                from_channel = re.match(r'^share\s+-from\s+(\S+)\s+-to\s+(\S+)$', text, re.IGNORECASE).group(1)
                to_channel = re.match(r'^share\s+-from\s+(\S+)\s+-to\s+(\S+)$', text, re.IGNORECASE).group(2)
                if self.operations_state["share"]["running"]:
                ...
                self.save_share_progress()
                self.operations_state["share"]["task"] = asyncio.create_task(self.share_videos(from_channel, to_channel))
        ...
        async def handle_all_messages(event):
            ...
            if not self.search_channel:
                ...
                await self.notify_admin(f"**🚫 لم يتم تحديد قناة بحث بواسطة @{username}**", is_search_notification=True)
                return
            if not self.video_db:
                ...
                await self.notify_admin(f"**🚫 قاعدة البيانات فارغة، المستخدم @{username} حاول البحث عن '{query}'**", is_search_notification=True)
                return
            ...
            if search_results:
                if len(search_results) == 1:
                    ...
                    for i, (title, video_id) in enumerate(search_results[:10], 0):  # حد أقصى 10 نتائج للتنسيق
                ...
                await self.notify_admin(f"**🚫 المستخدم @{username} بحث عن '{query}' ولم يتم العثور على نتائج**", is_search_notification=True)
