...
def format_media_message(media_data: Dict[str, Any], emoji: str = "💭") -> str:
    ...
    title = f"{arabic_title} | {english_title}"
...
async def handle_callback(update: Update, context: CallbackContext) -> None:
    ...
    parts = data.split('_')
    action = '_'.join(parts[:2]) if len(parts) >= 2 else parts[0]
    ...
    if action == "admin_add":
        ...
        if parts[2] == "start" and parts[3] == "message":
        ...
        if parts[2] == "start" and parts[3] == "image":
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("admin_users", handle_callback))
