...
import subprocess # Import for subprocess (FFmpeg)
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        # --- View Details (Movie/Series) ---
            ...
            # try: await message.delete() # <<< REMOVED DELETE MESSAGE HERE
            # except: pass # Ignore if deletion fails
            status_message = await message.edit_text(f"⏳ Fetching details for **{selected_item['title']}**...") # EDIT INSTEAD OF DELETE
            ...
            # Send result
            try:
                # await status_message.delete() # <<< REMOVED DELETE MESSAGE HERE - EDIT INSTEAD
                if image_url:
                    await status_message.edit_media( # EDIT MEDIA INSTEAD OF SENDING NEW PHOTO
                        media=InputMediaPhoto(media=image_url, caption=text, parse_mode=enums.ParseMode.MARKDOWN),
                        reply_markup=keyboard
                    )
                    # await client.send_photo( # <<< REPLACED BY EDIT_MEDIA
                    #     chat_id=user_id, photo=image_url, caption=text,
                    #     reply_markup=keyboard, parse_mode=enums.ParseMode.MARKDOWN
                    # )
                else:
                     await status_message.edit_text( # EDIT TEXT INSTEAD OF SENDING NEW MESSAGE
                        text=text, reply_markup=keyboard,
                        disable_web_page_preview=True, parse_mode=enums.ParseMode.MARKDOWN
                     )
                     # await client.send_message( # <<< REPLACED BY EDIT_TEXT
                     #    chat_id=user_id, text=text, reply_markup=keyboard,
                     #    disable_web_page_preview=True, parse_mode=enums.ParseMode.MARKDOWN
                     # )
            except Exception as send_err:
                 logger.warning(f"Failed to send/edit details (photo: {bool(image_url)}): {send_err}. Sending text fallback.")
                 try:
        ...
        # --- View Episode Details ---
            ...
            # try: await message.delete() # <<< REMOVED DELETE MESSAGE HERE
            # except: pass # Ignore if deletion fails
            status_message = await message.edit_text(f"⏳ Fetching details for **{episode_title}**...") # EDIT INSTEAD OF DELETE
...
async def download_and_send_video(client: Client, user_id: int, url: str, item_details: dict, progress_msg: Message):
    ...
    try:
        ...
        # Disable SSL verification by adding `verify=False` here for testing ONLY
        async with httpx.AsyncClient(verify=False).stream("GET", url, timeout=None, follow_redirects=True) as response: # <<< SSL DISABLED HERE
        ...
        try:
            thumb_path = os.path.join(DOWNLOAD_PATH, f"thumb_{message_identifier}.jpg")
            duration_cmd = ["ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", file_path]
            duration_output = subprocess.check_output(duration_cmd, stderr=subprocess.DEVNULL).decode('utf-8').strip()
            if duration_output and duration_output.replace('.', '', 1).isdigit(): # Check if it's a valid number
                duration = float(duration_output)
                thumb_time = duration / 2  # Middle of the video
                thumb_cmd = ["ffmpeg", "-ss", str(thumb_time), "-i", file_path, "-vframes", "1", "-vf", "scale=320:-1", thumb_path]
                subprocess.run(thumb_cmd, check=True, stderr=subprocess.DEVNULL) # Capture thumbnail silently
                logger.info(f"Thumbnail generated: {thumb_path}")
            else:
                logger.warning("Could not determine video duration for thumbnail generation.")
                thumb_path = None
        except Exception as ffmpeg_thumb_err:
            logger.warning(f"FFmpeg thumbnail generation failed: {ffmpeg_thumb_err}")
            thumb_path = None
        ...
        # thumbnail_url = item_details.get('image_url') # No longer using URL thumbnail
        # if thumbnail_url and not thumb_path: # Only download if FFmpeg failed, which is unlikely now
        #      try: # Redundant now, keeping for reference
        ...
        await client.send_video(
            ...
            progress_args=(progress_msg, f"⚡ Turbo Upload: {item_title[:15]}...", start_time), # <<< Turbo Upload text
            chunk_size=2 * 1024 * 1024,  # 2MB chunks for faster upload <<< Chunk size increased
            disable_notification=True, # Disable notifications during upload <<< No notifications
            allow_cache=False, # Prevent caching during upload <<< No cache
            read_timeout=30, # Increased timeouts <<< Increased timeouts
            write_timeout=30,
            connect_timeout=30
        )
